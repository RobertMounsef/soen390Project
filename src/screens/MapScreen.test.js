 ...
 * So every test that expects placeholders/📍 must call openSearchUI().
 */
...
describe('MapScreen', () => {
  ...
  describe('Search Functionality', () => {
    ...
    it('should clear origin query when text is cleared', async () => {
      const utils = render(<MapScreen initialShowSearch={true} />);
      await openSearchUI(utils);
    ...
    it('should clear destination query when text is cleared', async () => {
      const utils = render(<MapScreen initialShowSearch={true} />);
      await openSearchUI(utils);
    ...
    it('should clear origin building when clear button is pressed (lines 156-157)', async () => {
      const utils = render(<MapScreen initialShowSearch={true} />);
      await openSearchUI(utils);
    ...
    it('should clear destination building when clear button is pressed (lines 161-162)', async () => {
      const utils = render(<MapScreen initialShowSearch={true} />);
      await openSearchUI(utils);
      ...
      // second ✕ is usually destination clear
    ...
    it('should clear both origin and destination when clearRoute is called from DirectionsPanel', async () => {
      ...
      // testID mentioned in your comment
  ...
  describe('Use Current Building (US-2.2)', () => {
    ...
    it('should update origin automatically when user moves while originMode is current', async () => {
      ...
      fireEvent.press(utils.getByLabelText(/use current location as starting point/i));
      let mapView = utils.UNSAFE_getByType('MapView');
      expect(mapView.props.originBuildingId).toBe('EV');
      ...
      utils.rerender(<MapScreen initialShowSearch={true} />);
      mapView = utils.UNSAFE_getByType('MapView');
  ...
  describe('Search Toggle FAB', () => {
    it('should toggle search visibility when pressed', async () => {
      const utils = render(<MapScreen initialShowSearch={false} />);
